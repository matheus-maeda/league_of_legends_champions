...
export default ({ champions }) => {
  ...
  let [canChangeSkin, setCanChangeSkin] = useState(true);

  async function fetchChampion(champion) {
    let championSkinsFetch = [];
    await fetch(
      `https://ddragon.leagueoflegends.com/cdn/12.15.1/data/pt_BR/champion/${champion}.json`
    )
      .then((response) => response.json())
      .then(function (response) {
        setChampionResponse(response.data);
        setChampionName(response.data[champion]["id"]);
        championSkinsFetch = Object.keys(response.data[champion]["skins"]).map(
          (key) => [response.data[champion]["skins"][key]]
        );
  ...
  function loadSkillInformation(skill) {
    if (skill != "p") {
      setSkillInformation(
        championResponse[championName]["spells"][skill]["description"]
      );
      ...
      setActiveSkill(4);
      setSkillInformation(
        championResponse[championName]["passive"]["description"]
      );
      ...
      <div className="ChampionRow">
        {champions.map((element) => (
          <div
            className="champion"
            key={element[0].id}
            onClick={() => fetchChampion(element[0].id)}
          >
            <img
              src={`http://ddragon.leagueoflegends.com/cdn/12.15.1/img/champion/${element[0].image.full}`}
              style={{
                transform: championName == element[0].id ? "scale(1.2)" : "",
              }}
            />
            <p>{element[0].name}</p>
          </div>
      ...
      {championName != "" ? (
        <div className="ChampionInfo">
          <div className="championImage">
            <button onClick={() => changeSkin(0)} style={{visible: canChangeSkin ? "visible" : "none"}}>{"<"}</button>
            <img
              src={`http://ddragon.leagueoflegends.com/cdn/img/champion/loading/${championName}_${actualSkin}.jpg`}
            />
            <button onClick={() => changeSkin("1")}>{">"}</button>
          </div>
          <div className="ChampionName">
            {actualSkin === 0 ? (
              <h2>{championResponse[championName]["name"]}</h2>
            ) : (
              <h2>
                {
                  championResponse[championName]["skins"][actualSkinAux - 1][
                    "name"
                  ]
                }
              </h2>
